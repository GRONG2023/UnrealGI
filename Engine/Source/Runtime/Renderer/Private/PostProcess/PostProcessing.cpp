...
namespace
{
	TAutoConsoleVariable<bool> CVarEnablePostProcessTest(
		TEXT("r.PostProcessing.EnablePostProcessTest"),
		false,
		TEXT("EnablePostProcessTest.\n")
		TEXT(" (default: false)"),
...
void AddPostProcessingPasses(FRDGBuilder& GraphBuilder, const FViewInfo& View, int32 ViewIndex, const FPostProcessingInputs& Inputs)
{
	...
	const auto TranslatePass = [](ISceneViewExtension::EPostProcessingPass Pass) -> EPass
	{
		switch (Pass)
		{
			...
			case ISceneViewExtension::EPostProcessingPass::PostProcessTest: return EPass::PostProcessTest;
	...
	if (IsPostProcessingEnabled(View))
	{
		...
		PassSequence.SetEnabled(EPass::PostProcessTest, CVarEnablePostProcessTest.GetValueOnRenderThread());
		...
		if (PassSequence.IsEnabled(EPass::PostProcessTest)) {
		
			PostProcessTestInput postProcessTestInput;
			PassSequence.AcceptOverrideIfLastPass(EPass::PostProcessTest, postProcessTestInput.OverrideOutput);
			postProcessTestInput.inputTexture = SceneColor;
			SceneColor = AddPostProcessTestPass(GraphBuilder, View, postProcessTestInput);
		}
		
		SceneColor = AddAfterPass(EPass::PostProcessTest, SceneColor);
